...
const StreamCount = styled.span`
  font-size: 1rem;
  background: ${(props) =>
    props.isLive ? "var(--live-color)" : "var(--primary-color)"};
  color: white;
...
const createStreamObject = (isLive = true) => {
  const title = isLive
    ? getRandomItem(liveStreamTitles)
    : getRandomItem(recordedStreamTitles);
  const university = getRandomItem(universities);
  const viewers = Math.floor(Math.random() * 10000);
  const duration = `${Math.floor(Math.random() * 2 + 1)}:${String(
...
const LiveList = () => {
  ...
  const initializeStreams = () => {
    const initialStreams = {
      live: Array(12)
        .fill()
        .map(() => createStreamObject(true)),
      recorded: Array(12)
        .fill()
        .map(() => createStreamObject(false)),
    };
  ...
  const filterByCategory = (streamArray, category) => {
    ...
    return streamArray.filter((stream) => {
      ...
      switch (category) {
        ...
        case "culture":
          return (
            title.includes("văn hóa") ||
            title.includes("festival") ||
            title.includes("giao lưu")
          );
        case "sports":
          return (
            title.includes("thể thao") ||
            title.includes("giải đấu") ||
  ...
  const handleLoadMore = (type) => {
    setStreams((prev) => ({
      ...
      [type]: [
        ...
        ...Array(4)
          .fill()
          .map(() => createStreamObject(type === "live")),
      ],
    }));
  };
  ...
  return (
    <>
      ...
      <MainContainer>
        ...
        <StreamSection type="live" title="Đang Live" icon={faBroadcastTower} />

        <StreamSection type="recorded" title="Live Đã Phát" icon={faClock} />
      </MainContainer>
    </>
  );
};
